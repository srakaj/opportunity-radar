...
DEADLINE_MARKERS = (
    ...
    r"applications? (?:are )?(?:accepted )?until",
    r"no later than",
    r"bewerbungsfrist",
    r"bewerben bis",
    r"bewerbungsschluss",
...
def _extract_deadline_context(description: str, existing_deadline: str = "") -> tuple[str, str]:
    ...
    for marker in marker_re.finditer(text):
        ...
        end = min(len(text), marker.end() + 180)
...
def _parse_time(context: str) -> tuple[time | None, str]:
    ...
    if match.group("word"):
        ...
        return (time(0, 0) if word == "midnight" else time(12, 0)), match.group(0)
...
def _explicit_timezone(context: str):
    ...
    if zone_name:
        return ZoneInfo(zone_name), token
    return None, ""


def infer_timezone_from_location(location: str) -> str:
    value = _clean(location).lower()
    ...
    state_match = re.search(r",\s*([A-Z]{2})(?:\b|$)", _clean(location))
...
def _status(deadline_utc: datetime | None, now: datetime | None = None) -> str:
    ...
    remaining = deadline_utc - current
...
def enrich_deadline_timezone(item: dict, now: datetime | None = None) -> dict:
    ...
    Confidence values:
    - explicit_timezone: source supplied both a time and timezone
    - location_inferred: source supplied a time; timezone inferred from job location
    - date_only: source supplied only a date; 23:59 Europe/Berlin assumed
    - timezone_unknown: source supplied a time but source timezone could not be inferred;
      time is treated as Europe/Berlin and clearly marked as an assumption
    """
    enriched = dict(item)
        ...
        enriched.update(
            {
                "deadline_berlin_iso": "",
                "deadline_utc_iso": "",
                "deadline_time_confidence": "unknown",
    ...
    if parsed_time is None:
        ...
        source_tz = BERLIN
        source_tz_label = "Europe/Berlin"
        confidence = "date_only"
    elif explicit_tz is not None:
        source_tz = explicit_tz
        source_tz_label = tz_label
        confidence = "explicit_timezone"
    else:
        inferred_zone = infer_timezone_from_location(_clean(enriched.get("location")))
        if inferred_zone:
            source_tz = ZoneInfo(inferred_zone)
            source_tz_label = inferred_zone
            confidence = "location_inferred"
        else:
            source_tz = BERLIN
            source_tz_label = "Europe/Berlin (fallback)"
            confidence = "timezone_unknown"
